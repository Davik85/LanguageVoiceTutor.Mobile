...
Planning checkpoint: 2026-09-20. Current Mobile source is `3389786c16d676dae2a66a9cdf73367780babcad` with `0.1.0+10` / versionCode 10 for package `com.languagevoicetutor.mobile`, and v10 is publicly active in Google Play Production. Production backend remains separate from this Mobile release; the earlier backend-owned multilingual Lesson Hint correction and its Android v9 verification required no Mobile Hint contract change. Password authentication remains authoritative; Restore Credentials is an additive convenience path that never transfers an existing refresh token, creates only the normal new backend session after verification, is suppressed by logout, and is removed with its public-credential and ceremony state during account anonymization. FlutterSecureStorage session state is device-bound and excluded from Android cloud backup, device transfer, and legacy full backup.

Practice reminders remain a confirmed non-critical open issue after public v10. V10 shipped without a practice-reminder implementation change and did not fix or reverify this behavior. Settings can show Daily practice reminders enabled and Android notification permission/channel allowed, but updating reminders can return `Unable to update reminders right now. Please try again.` with no scheduled Orralen reminder alarms remaining. Current Mobile reconciliation cancels existing reminders before scheduling replacements, so a later scheduling failure can leave no reminders; the client exposes only the generic failure result. The underlying Android/plugin exception and exact root cause are not yet proven, including any exact-alarm theory. Correction and verification require a separately approved future Mobile maintenance release; this does not justify rolling back v10.
...
The current baseline includes completed backend-owned Lesson History, Android lesson/voice flows, authentication/session resilience, Feedback & reports, the Notifications V1 foundation, localization, and backend-owned Progress. Settings has stable **Account**, **Learning**, and **Audio** areas; Home follows **Choose Topic -> Choose Situation -> Lesson** using the backend-owned selected level. The bounded practice-reminder scheduling failure remains open after v10 for a future separately approved maintenance release, so reminders are not an entirely green current baseline. Google Play Billing, RTDN, and reconciliation are active in public Production. Analytics, crash reporting, future artifact changes, and post-release lifecycle monitoring remain separate work.
...
Current source is `0.1.0+10` / versionCode 10 at `3389786c16d676dae2a66a9cdf73367780babcad`. Google Play accepted and published version 10 as the active Production release on 2026-09-20. The signed `app/build/app/outputs/bundle/release/app-release.aab` is 196468474 bytes with SHA-256 `45B2552ACE707A94772AC0739BF7B6E8632A8C994A54D33C3759DDF79B842C0C`; `jarsigner` reported `jar verified`, its embedded upload-certificate SHA-256 is `36:40:5D:B4:56:47:B2:3C:68:EE:2D:AB:12:21:70:CA:DE:06:11:38:28:D9:9D:02:AB:62:54:33:E2:F5:0B:F7`, and target SDK remains 36. Versions 8 and 9 remain historical public releases; their dated validation evidence is not relabeled as v10 evidence.
...
The current v10 Production page shows two non-blocking post-release quality recommendations: edge-to-edge compatibility and R8 memory/performance optimization. They are recommendations, not policy errors or release failures. Physical checks found no visible edge-to-edge boundary problem on the tested Android 15/16 Samsung S25, Samsung A56, and Honor devices; that evidence is limited to those devices and is not a universal compatibility claim. Edge-to-edge and R8 changes require separate implementation and verification.
...
The current v10 output is `app/build/app/outputs/bundle/release/app-release.aab` (196468474 bytes), SHA-256 `45B2552ACE707A94772AC0739BF7B6E8632A8C994A54D33C3759DDF79B842C0C`; `jarsigner` reported `jar verified`. It was built from `3389786c16d676dae2a66a9cdf73367780babcad`, carries the same verified upload certificate SHA-256 recorded above, and is publicly active in Google Play Production as of 2026-09-20.
