...
Planning checkpoint: 2026-09-09. Current Mobile source is `0.1.0+9` / versionCode 9 for package `com.languagevoicetutor.mobile`, and v9 is publicly available in Google Play Production. Production backend is `0.1.35-backend.156` with `.155` rollback. Backend `.156` fixed the backend-owned multilingual Lesson Hint contract and was manually verified with Android v9 on several non-English study languages; no Mobile Hint code change or Mobile rebuild was required. Password authentication remains authoritative; Restore Credentials is an additive convenience path that never transfers an existing refresh token, creates only the normal new backend session after verification, is suppressed by logout, and is removed with its public-credential and ceremony state during account anonymization. FlutterSecureStorage session state is device-bound and excluded from Android cloud backup, device transfer, and legacy full backup.

Practice reminders are a confirmed non-critical public-v9 follow-up for the next Mobile maintenance release / v10. Settings can show Daily practice reminders enabled and Android notification permission/channel allowed, but updating reminders can return `Unable to update reminders right now. Please try again.` with no scheduled Orralen reminder alarms remaining. Current Mobile reconciliation cancels existing reminders before scheduling replacements, so a later scheduling failure can leave no reminders; the client exposes only the generic failure result. The underlying Android/plugin exception and exact root cause are not yet proven, including any exact-alarm theory. This does not justify rolling back v9.
...
The current baseline includes completed backend-owned Lesson History, Android lesson/voice flows, authentication/session resilience, Feedback & reports, the Notifications V1 foundation, localization, and backend-owned Progress. Settings has stable **Account**, **Learning**, and **Audio** areas; Home follows **Choose Topic -> Choose Situation -> Lesson** using the backend-owned selected level. The bounded practice-reminder scheduling failure above remains an open v10 maintenance item, so reminders are not an entirely green public-v9 baseline. Google Play Billing, RTDN, and reconciliation are active in public Production. Analytics, crash reporting, future artifact changes, and post-release lifecycle monitoring remain separate work.
...
Current source is `0.1.0+9` / versionCode 9 at `0af802958a6a42116ecc1d8084ebdecc12e11406`. Google Play accepted its signed AAB into the Production workflow, and v9 is publicly available in Production as of 2026-09-09. Version 8 remains a historical public release; its dated validation evidence below is not relabeled as v9 evidence.
...
The current v9 output is `app/build/app/outputs/bundle/release/app-release.aab` (196311466 bytes), SHA-256 `C301B36333FB8B70AB8A7372D5E74BA62AC9810AD6704730234BCBBE951B703C`; `jarsigner` reported `jar verified`. It was built from `0af802958a6a42116ecc1d8084ebdecc12e11406` and is publicly available in Google Play Production. The Free Conversation production regression was corrected earlier in `fefbac3f366920b980f831ddd55c18e2734471a0`: only initial `setupContextSelection` uses scenario selection, while Free Conversation active-roleplay voice and Hint use normal paths; physical Android verification also retained ordinary scenario selection.
