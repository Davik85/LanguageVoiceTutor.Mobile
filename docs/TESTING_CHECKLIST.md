...
- For the original six-study-language slice, confirm no interface behavior, client OpenAI call, backend endpoint, CMS prompt, or backend deployment changed. The later Stage 1 interface checkpoint intentionally adds ARB/`flutter_localizations` presentation resources without changing lesson-language contracts.
...
- Post-release v8 phase-boundary regression check: Free Conversation enters active roleplay without a selected context. Its Lesson Chat Auto-send and Conversation mode voice transcripts use the normal reply path without scenario resolution; Hint uses the active-roleplay example or backend Hint path, never the setup-context hint. The corrective commit is `fefbac3f366920b980f831ddd55c18e2734471a0`; `flutter analyze`, both focused voice/lesson suites, the full Flutter suite, and `git diff --check` passed before commit, followed by physical Android verification of Free Conversation Hint, Auto-send, Conversation mode, and ordinary scenario selection.
- V10 request-boundary regression check: `3389786c16d676dae2a66a9cdf73367780babcad` preserves that phase boundary and additionally ensures ordinary Free Conversation learner text with no committed context is sent as the normal `UserMessage` only, with no duplicate `selectedContextTitle`. The first normal turn has `lessonPhase=active_roleplay`, `learnerTurnCount=1`, `isContextSelectionTurn=false`, and empty `selectedContextVariantId` and `selectedContextTitle`. Real CMS scenario and real custom-context flows remain unchanged; Conversation Mode production rendering was unchanged and only its stale assertion was corrected. Focused affected suites and physical Android smoke for Free Conversation plus an ordinary guided lesson with a selected situation passed.
...
- [x] Google Play accepted and published version 10 as the active Production release on 2026-09-20. Historical v8/v9 release and validation evidence remains dated and is not relabeled as v10 evidence.
- [x] The final release gate completed clean repository verification; `flutter clean`; `flutter pub get`; `flutter gen-l10n`; `flutter analyze`; the full Flutter suite with concurrency 1; a clean-source check; `flutter build appbundle --release`; Android versionName/versionCode verification; SHA-256, `jarsigner`, and upload-certificate verification; and final clean repository / `HEAD == origin/main` verification. No unverified total test count is recorded.
...
- Google Play Console verification complete: package `com.languagevoicetutor.mobile` is registered, Android developer verification is confirmed, and no additional package or key registration is currently required.
- Physical Internal testing complete on five Samsung and Huawei Android devices: Google Play installation; system-language startup plus native/interface defaults; registration; restoration of backend-owned settings, Progress, and History; lesson scenario completion; Conversation Mode; speech recognition and automatic sending; tutor voice playback; Hints; Translation; Feedback; Summary; Progress; general and topic-specific achievements; study-language switching without mixed-language lesson openings; tutor avatar/voice changes; Settings History/Progress; password change/recovery; background/resume; force-close/reopen; temporary network loss/recovery; and microphone-permission denial/recovery all passed.
- Non-blocking pre-release polish: after microphone permission is denied, Lesson Chat keeps the microphone-denied/open-settings warning visible. Before final public release, show it only when the learner attempts to use the microphone while permission remains denied; do not treat this as a release-blocking functional defect.
- Historical upload-key reset requests are not a current blocker because the working upload key matches the existing AAB certificate. Do not request another reset or cancellation without a new concrete reason.
...
- Selecting a situation reconstructs canonical level, topic, subtopic, situation, context, and `lessonContentId` values in `LessonStartSelection`; localized labels remain display-only.
