...
The start-flow fixture must provide the Home requests it triggers: a permitted lesson-access result and unavailable Progress and Achievements results. It must not attempt to navigate to `/login`; that route is intentionally absent from this focused test app. Current result: `lesson_start_flow_test.dart` passes with 59 tests. The lesson visual boundary is local to `LessonScreen`: Home and Settings retain product-blue typography, while feedback and summary retain their own neutral, sectioned presentation.
