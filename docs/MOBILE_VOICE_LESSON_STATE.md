...
English, French, German, Portuguese, Spanish, and Italian resolve through the same centralized Mobile study-language definition. Lesson Chat and Conversation mode transcription and TTS send the same exact target-language metadata, independent of native/translation and interface languages. Initial setup and known-context confirmation use valid response-owned `localizedSetup`; Mobile renders only the user display name into its template and resolves titles by stable ID. Missing or invalid non-English projections use packaged target-language fallback rather than canonical English. Pre-context Hint and the first CMS example Hint retain their existing deterministic target-language behavior. Voice candidate requests retain canonical candidate IDs and canonical English titles and use localized titles only as learner-facing candidate context. Exact localized titles can resolve the same canonical runtime variant; published/free/clarify/unsafe/failure semantics are unchanged. Interface localization now selects fourteen application locales independently; it does not change the six study-language transcription or TTS behavior. The obsolete voice-transcription disclosure has been removed. Backend/CMS ownership is unchanged, and no backend deployment was required.
...
The lesson experience separately handles known CMS contexts, valid learner-supplied custom contexts, and Free Conversation. Known and valid custom contexts both start locally at learner turn zero with no normal lesson-reply call; Free Conversation begins directly in active roleplay and uses normal replies for actual learner turns. Invalid or unresolved context input keeps the existing clarification behavior. Hint behavior remains backend-owned and uses the selected lesson context after a scenario has started. Audio playback handles completion and recovery so voice controls return to a usable state after playback ends or fails. Tutor avatar state is handled with preload fallback rather than assuming every possible lesson-chat avatar asset is present. Keyboard-open layout keeps the composer and **Send** button visible without `RenderFlex` overflow.

On 2026-09-06, `fefbac3f366920b980f831ddd55c18e2734471a0` (`Fix Free Conversation voice scenario handling`) enforced this existing boundary: only `setupContextSelection` without a selected context is an initial voice scenario-selection turn. Free Conversation remains `activeRoleplay` without a selected context, so its Hint and Lesson Chat/Conversation mode voice turns follow their active-roleplay paths rather than scenario resolution. Automated verification and physical Android regression checks passed for Free Conversation Hint, voice Auto-send, Conversation mode, and ordinary scenario selection. Backend/CMS ownership and the voice-scenario API contract did not change.

On 2026-09-20, `3389786c16d676dae2a66a9cdf73367780babcad` (`Fix Free Conversation context handling`) added a separate narrow request-boundary correction without replacing or reverting `fefbac3`. During Free Conversation active roleplay with no real committed context, ordinary learner text remains the normal `UserMessage` and is no longer also copied into the transient `selectedContextTitle`. The first normal turn therefore has `lessonPhase=active_roleplay`, `learnerTurnCount=1`, `isContextSelectionTurn=false`, and empty `selectedContextVariantId` and `selectedContextTitle`. Real CMS scenario selection and real custom-context flows remain unchanged. Conversation Mode production rendering was not changed; only its stale test assertion was corrected. Focused affected suites passed, and physical Android smoke passed for Free Conversation normal conversation plus an ordinary guided lesson with a selected situation; the final full v10 release gate subsequently passed. No backend, API, CMS, or ownership-model change was required.

## Live lesson state and bounded context

Mobile uses centralized live phases for setup/context selection, active roleplay, wrap-up, final, and completed-awaiting-Finish. Selecting a known or custom context remains learner turn zero. The backend response owns valid localized setup and known-context confirmation text; the authenticated learner display name is the only template replacement. Custom-context opening remains local target-language behavior. English stays canonical, and invalid response localization never causes a canonical-English fallback for a non-English lesson.

Normal learner turns are committed only after a successful normal reply. The first qualifying turn enters wrap-up, later turns remain in wrap-up, and the hard-final learner turn is completed locally with the CMS final message and enabled **Finish** action rather than a normal reply request. The current learner input is sent separately and is not duplicated in prior history.
