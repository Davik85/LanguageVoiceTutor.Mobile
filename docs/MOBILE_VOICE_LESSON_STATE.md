...
Mobile uses centralized live phases for setup/context selection, active roleplay, wrap-up, final, and completed-awaiting-Finish. Selecting a known or custom context remains learner turn zero. The English CMS setup message, known-context confirmation/opening, and custom-context opening follow Desktop-visible-text behavior locally; the authenticated learner display name and runtime tutor display name are supplied to the existing request/opening paths, including tutor-name placeholders.
