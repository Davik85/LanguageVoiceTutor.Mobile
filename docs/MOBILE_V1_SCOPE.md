...
The Android manifest now declares the app-side Digital Asset Links `asset_statements` include for `https://languagevoicetutor.com/.well-known/assetlinks.json`; the website-hosted `assetlinks.json`, including the verified Play App Signing certificate fingerprint, remains unpublished and pending. This is local source work only. Backend deployment, migration application/configuration, Android Studio/device restore E2E, and a v7 Internal-testing upload remain pending; v7 has not been uploaded to Google Play.
...
Boundaries remain: no CMS report-review screen exists yet, no email workflow exists, no attachments or screenshots are supported, no report button was added to individual chat messages, no automatic moderation or OpenAI forwarding exists, no new Mobile database was created, backend remains the owner of persistence and authenticated `UserId`, and a future CMS list with `new`, `reviewed`, and `resolved` states remains separate work. Remaining interface localization, analytics, crash reporting, and final store release work remain separate. Release signing, the Play package registration, and Android developer verification are complete.
