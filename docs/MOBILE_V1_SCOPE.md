...
## Android v9 public Production checkpoint (2026-09-09)
...
Practice reminders are an open, non-critical Mobile v10 follow-up. Public-v9 evidence is bounded: Settings can show reminders enabled and notification permission/channel allowed, yet updating reminders can show `Unable to update reminders right now. Please try again.` and leave no scheduled Orralen reminder alarms. Current reconciliation cancels existing reminders before scheduling replacements, and the client hides the underlying scheduling exception behind a generic result. The exact Android/plugin cause is unconfirmed; no exact-alarm or other speculative root cause is claimed, and this defect is not a v9 rollback reason.
