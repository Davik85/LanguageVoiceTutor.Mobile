...
## Restore Credentials v8 validation and public-production availability (2026-09-03)

Mobile source first validated through Google Play Internal testing and now publicly available as `0.1.0+8` / versionCode 8 contains an Android-only Google Credential Manager Restore Credentials bridge using stable AndroidX Credentials `1.6.0`. Existing password authentication, JWT/refresh-token storage, and billing behavior remain authoritative and unchanged. After manual password authentication, the app best-effort exchanges only backend-provided WebAuthn registration options and public credential responses; no refresh token or credential private material is transferred or persisted by Flutter.

At startup a valid existing local session always wins. If normal authentication is required, the app silently attempts one Restore Credentials assertion and persists the ordinary backend `AuthResponse` token pair only after backend verification succeeds; disabled backend, unavailable/unsupported devices, absent keys, and rejected assertions fall back to Login. Explicit logout records local automatic-restore suppression before attempting native restore-key clear, clears the user-scoped sync marker and the normal session, and manual password authentication clears that suppression.
...
## Historical Android v9 public Production checkpoint (2026-09-09)

Current source is `0af802958a6a42116ecc1d8084ebdecc12e11406` with `0.1.0+9` / versionCode 9, and v9 is publicly available in Google Play Production. Version 8 remains a historical public release. This checkpoint does not retest or relabel the historical v8 Restore Credentials account/session validation as v9 evidence.

Production backend is `0.1.35-backend.156` with `.155` retained for rollback. `.156` fixed the backend-owned multilingual Lesson Hint contract: Hint output is locked to the selected study language, deterministic fallback covers English, French, German, Portuguese, Spanish, and Italian, and missing or unknown ids retain the English compatibility default. Android v9 production smoke passed on several non-English study languages, including repeated Hint requests. No Mobile code change or rebuild was required for this backend correction.

Practice reminders were recorded as an open, non-critical Mobile follow-up during public v9 and remain open after public v10. Settings can show reminders enabled and notification permission/channel allowed, yet updating reminders can show `Unable to update reminders right now. Please try again.` and leave no scheduled Orralen reminder alarms. Current reconciliation cancels existing reminders before scheduling replacements, and the client hides the underlying scheduling exception behind a generic result. V10 shipped without a reminder implementation change and did not fix or reverify this behavior. The exact Android/plugin cause is unconfirmed; no exact-alarm or other speculative root cause is claimed. Correction and verification require a separately approved future Mobile maintenance release.

## Android v10 public Production checkpoint (2026-09-20)

Current source is `3389786c16d676dae2a66a9cdf73367780babcad` with `0.1.0+10` / versionCode 10, and v10 is publicly active in Google Play Production for package `com.languagevoicetutor.mobile`. The signed `app/build/app/outputs/bundle/release/app-release.aab` is 196468474 bytes with SHA-256 `45B2552ACE707A94772AC0739BF7B6E8632A8C994A54D33C3759DDF79B842C0C`; `jarsigner` reported `jar verified`, the embedded upload-certificate SHA-256 is `36:40:5D:B4:56:47:B2:3C:68:EE:2D:AB:12:21:70:CA:DE:06:11:38:28:D9:9D:02:AB:62:54:33:E2:F5:0B:F7`, and target SDK remains 36. The full clean v10 release gate and final `HEAD == origin/main` verification passed without asserting an unverified total Flutter test count. Historical v8/v9 evidence remains dated evidence rather than being relabeled as v10 validation.

V10 includes the Mobile-only authentication onboarding presentation update from `2457f25f2928b95edba5179aa26e4131db60de56`: the Login/Register screen uses the new Orralen hero artwork, existing application gradient, and a soft fade; removes the redundant auth AppBar title and old separate Login Orralen logo/wordmark block; and preserves the localized three-sentence welcome copy, authentication, registration, password recovery, scrolling, and keyboard behavior. It does not change the authentication backend contract or claim a complete product-wide rebrand.

The earlier `fefbac3f366920b980f831ddd55c18e2734471a0` phase-boundary correction remains intact: Free Conversation starts in `activeRoleplay`, while only initial `setupContextSelection` uses scenario selection. Source `3389786c16d676dae2a66a9cdf73367780babcad` adds a narrow request-boundary correction so ordinary Free Conversation learner text with no committed context remains the normal `UserMessage` and does not also populate `selectedContextTitle`. Real CMS scenario selection and real custom-context flows remain unchanged; Conversation Mode production rendering was not changed. Focused affected suites and physical Android smoke passed for Free Conversation normal conversation and an ordinary guided lesson with a selected situation, followed by the full v10 release gate. No backend, API, CMS, billing, entitlement, Product ID, Base Plan ID, RTDN, reconciliation, or backend-verification rule changed for this Mobile release.

## Interface localization and language ownership

Mobile selects fourteen interface locales: `en`, `ru`, `es`, `fr`, `de`, `it`, `pt-PT`, `bg`, `hr`, `sr-Latn`, `pl`, `ja`, `ko`, and `ar`. The selectable catalogs contain 454 messages each, including the localized authentication welcome message. Generated `pt` and `sr` are internal `gen-l10n` fallbacks, producing 16 generated variants but not more selectable locales.

The six study languages (`en`, `fr`, `de`, `pt`, `es`, `it`) remain a separate lesson boundary. `studyLanguage` controls lessons, transcription, and tutor audio; `nativeLanguage` is the translation target; `explanationLanguage` controls the interface. The fixed-LTR application shell is deliberate for every interface locale, including Arabic localized text.

On first run, Android's ordered locale list resolves independent interface and native defaults, each with an English fallback; it never changes `studyLanguage`. Splash and Login use the device-derived interface before authentication. Backend `UserSettings` override it after authentication. Existing account login never writes settings. New-account registration fetches the created settings and updates only native and explanation language, preserving study language, voice, speed, conversation mode, tutor, and level; failed follow-up settings calls do not fail authentication or retry registration. No marker, duplicate local settings store, backend contract, migration, dependency, Gradle, Kotlin, or backend deployment is required.

The safe lesson catalog separates presentation from canonical data. Stable topic IDs select the canonical situation list; unknown display IDs fall back to canonical text. `LessonStartSelection` resolves canonical level and situation objects by stable ID before creating backend-facing values. Consequently translated/copied display objects cannot alter IDs, English titles, `lessonContentId`, session requests, or runtime scenario keys. Request payloads and scenario keys are identical in all selectable interface locales, `scenarioKey` remains `lessonContentId`, and Free Conversation keeps its canonical runtime path.

## Supported lesson languages

Mobile V1 lesson practice supports `en`, `fr`, `de`, `pt`, `es`, and `it` through one authoritative study-language catalog. The lesson-start response owns the authoritative localized setup projection: Mobile accepts nullable `localizedSetup` only when its selected non-English language matches, its template is nonblank, its placeholders exactly match, and its stable-ID context-title coverage is exact and nonblank. A valid template is rendered with only the user display name and its formatting preserved; missing or invalid response data uses packaged target-language fallback, never canonical English. Stable CMS IDs, canonical English titles, runtime variants, and scenario keys remain canonical backend metadata. Context confirmation resolves localized titles from stable IDs for numeric, local, canonical, and alias selection. Session start keeps the backend-compatible English language name; reply/Hint/Feedback requests, transcription, and TTS carry the centralized ID, English name, native name, and language code. Native/translation and interface languages do not select the practiced language. The fourteen-locale interface implementation is independent from this six-language lesson behavior. Commit `01d6226` completed this Mobile integration; no backend/CMS deployment was required by the client change.

## Current verified baseline

Latest known functional learner microphone recording and speech-to-text commit: `e2ec9d0cdb88b6eab8b1100d46188963e05f723b` (`Add mobile speech recording and transcription`). Latest known functional Feedback commit: `f1e8f16` (`Add mobile learner message feedback`). Latest known functional Translation commit: `9d2476b` (`Add mobile message translation`). Latest known settings language persistence fix: `340c950` (`Fix mobile settings language persistence`). The Flutter Android client under `app/` has a production-verified text lesson loop, completed mobile Hint flow, shared Lesson Chat/Conversation mode transcription request building for Desktop-parity transcription behavior, and green Settings parity/Home polish baseline. Settings has stable visible **Account**, **Learning**, and **Audio** sections, and **Save settings** is visible and tested. **Learning** reads and saves the backend-owned account `CurrentLevel`; `lessonLevels` remains the centralized Mobile display and mapping collection. Home Start lesson loads that setting and opens Choose Topic directly, so Choose Level is no longer part of the normal flow. Selected tutor also persists through the backend settings API. CMS-published level profiles remain the source of lesson behavior and timing; Mobile does not own lesson complexity or duration rules. No backend deployment was required because backend release `0.1.35-backend.116` already provided the required `CurrentLevel` settings contract, and the owner physically verified this saved-level lesson-start slice on an Android phone.
...
Boundaries remain: no CMS report-review screen exists yet, no email workflow exists, no attachments or screenshots are supported, no report button was added to individual chat messages, no automatic moderation or OpenAI forwarding exists, no new Mobile database was created, backend remains the owner of persistence and authenticated `UserId`, and a future CMS list with `new`, `reviewed`, and `resolved` states remains separate work. Analytics, crash reporting, and future artifact-change work remain separate. Release signing, the Play package registration, Android developer verification, and public Android v8 publication are complete.
...
Study language, native/translation language, and interface language are separate concepts. Mobile has an explicit study-language definition containing ID, English name, native name, and transcription language code; the six study languages remain English, French, German, Portuguese, Spanish, and Italian. Stage 1 interface localization currently supports `en`, `ru`, `es`, `fr`, and `de`, while the long-term approved list remains `en`, `es`, `fr`, `de`, `it`, `pt`, `ru`, `pl`, `ar`, `ja`, `ko`, `sr`, `hr`, and `bg`. Localization never translates AI tutor replies, learner messages, backend-generated content, CMS identifiers, canonical scenario keys, internal IDs, or backend data. Remaining languages and Arabic right-to-left verification are later work.
...
- Google Play public-production monitoring: controlled Internal-testing purchase, renewal reconciliation, and final expiry, plus the 2026-09-01 real-money first purchase and backend-owned initial Premium/trial deferral, are verified; v10 is publicly active, while v8 and v9 remain historical. V10 did not newly retest billing lifecycle scenarios. The actual real-money renewal charge and broader lifecycle observations remain pending.
- Apple billing.
