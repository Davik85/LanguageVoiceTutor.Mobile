...
Study language, explanation/translation language, and interface language are separate concepts. Mobile has an explicit study-language definition containing ID, English name, native name, and transcription language code; the six study languages remain English, French, German, Portuguese, Spanish, and Italian. Interface localization has not started, but its approved 14-language scope is `en` English, `es` Spanish, `fr` French, `de` German, `it` Italian, `pt` Portuguese, `ru` Russian, `pl` Polish, `ar` Arabic, `ja` Japanese, `ko` Korean, `sr` Serbian, `hr` Croatian, and `bg` Bulgarian. On first use, interface and explanation/translation language default from the device language, normalizing supported variants such as `en-US` to `en`, `pt-BR` to `pt`, and `sr-Latn-RS` to `sr`; unsupported device languages fall back to English. Once the learner explicitly saves language settings, backend-owned saved settings override future automatic detection. Localization will cover static UI, validation, notification, Premium, and billing text only, never AI tutor replies, learner messages, backend-generated content, CMS identifiers, canonical scenario keys, internal IDs, or backend data. Arabic requires right-to-left verification. The native/explanation-language catalog remains broader than the study-language catalog, and Settings stores and sends backend language IDs.
...
Premium UI is a separate stage before Billing. A button, local callback, or Play Store result never grants Premium. Google Play Billing is complete only with purchase-token submission, backend verification, entitlement refresh, restore/reconciliation, and relevant subscription lifecycle states. Paddle remains unchanged for website/desktop; valid provider purchases map through the shared provider-neutral backend entitlement model and remain visible to other clients. Localization intentionally follows Notifications, Premium UI, and billing so their text is included in one complete pass; do not add ARB files, `flutter_localizations`, or translated strings until that stage.
