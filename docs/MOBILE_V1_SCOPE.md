...
Flutter `gen-l10n` and ARB files now provide the Mobile interface-localization foundation for `en`, `ru`, `es`, `fr`, and `de`. Every catalog has the same 277-message key set. `explanationLanguage` alone selects `MaterialApp.locale`; `studyLanguage` remains responsible for lessons, tutor behavior, recognition, and speech, and `nativeLanguage` remains the lesson/dialogue translation target. Unsupported interface values fall back visually to English without overwriting the saved backend value. A confirmed Settings save applies the interface language immediately, while a failed save keeps the prior locale.

Completed localization includes Splash/authentication, Home, Settings Profile/Lessons/App, account and deletion requests, Feedback & reports, reminders, fixed level display, and localized topic/situation selection. It includes the completed areas' headings, helper text, validation, semantics, and tooltips. The removed voice-transcription disclosure is not part of the interface. Premium, Progress, Lesson History details, remaining Achievement catalogue text, Lesson Chat, Conversation mode, and other static strings remain later work; Stage 1 is not whole-app complete.

The safe lesson catalog separates presentation from canonical data. Stable topic IDs select the canonical situation list; unknown display IDs fall back to canonical text. `LessonStartSelection` resolves canonical level and situation objects by stable ID before creating backend-facing values. Consequently translated/copied display objects cannot alter IDs, English titles, `lessonContentId`, session requests, or runtime scenario keys. Request payloads and scenario keys are identical in all five interface locales, `scenarioKey` remains `lessonContentId`, and Free Conversation keeps its canonical runtime path.

## Supported lesson languages

Mobile V1 lesson practice supports `en`, `fr`, `de`, `pt`, `es`, and `it` through one authoritative study-language catalog. Tutor-facing local lesson setup, scenario labels, known-context roleplay openings, and local Hints follow the selected study language. Session start keeps the backend-compatible English language name; reply/Hint/Feedback requests, transcription, and TTS carry the centralized ID, English name, native name, and language code. Native/translation and interface languages do not select the practiced language. Stable CMS IDs, canonical English titles, runtime variants, and scenario keys remain canonical backend metadata. The five-locale interface foundation is independent from this six-language lesson behavior; no backend/CMS deployment was required.
...
5. Continue the verified five-locale Stage 1 foundation across the remaining static screens and approved languages.
