...
Mobile V1 lesson practice now supports `en`, `fr`, `de`, `pt`, `es`, and `it` through one authoritative study-language catalog. Tutor-facing local lesson setup, scenario labels, known-context roleplay openings, and local Hints follow the selected study language. Session start keeps the backend-compatible English language name; reply/Hint/Feedback requests, transcription, and TTS carry the centralized ID, English name, native name, and language code. Native/translation language and interface language do not select the practiced language. Stable CMS IDs, canonical English titles, runtime variants, and scenario keys remain canonical backend metadata. UI localization is explicitly outside this phase, no backend/CMS deployment is required, and device verification of all six language paths remains required.
