...
## Historical controlled validation and current public-production checkpoint (updated 2026-09-20)
...
Current Mobile `0.1.0+10` / versionCode 10 from source `3389786c16d676dae2a66a9cdf73367780babcad` is publicly active in Google Play Production as of 2026-09-20. V10 did not change or newly validate the billing architecture, Product ID `premium`, Base Plan ID `monthly`, backend-owned entitlement authority, RTDN, reconciliation, acknowledgement ownership, purchase verification, or fail-closed rules. It also does not relabel historical controlled billing evidence as v10 testing. Signed-out password recovery was first distributed publicly in v8 and remains present in current v10.
...
Historical controlled license-test purchase, renewal, and final expiry are proven. The 2026-09-01 real-money first purchase and backend-owned initial Premium/trial deferral are also proven. Android v10 is now publicly active in Production, while the v8/v9 checkpoints remain historical; remaining lifecycle evidence is post-release monitoring, not a current-release validation claim. Actual real-money renewal scheduled for 2026-10-08, pending-payment handling, explicit cancellation before natural expiry, restore on a fresh installation, refund/voided-purchase lifecycle, chargeback lifecycle, and provider-isolation edge cases beyond existing automated/backend coverage remain unobserved.

This is now post-release monitoring: periodically re-review legal/public-policy wording, Google Play Data Safety, test-only controls (including `TestPurchasesEnabled`), pending-refund-review decision, monitoring, and rollback readiness. Signed-out password recovery was first distributed in public `0.1.0+8` / versionCode 8 and remains present in public `0.1.0+10` / versionCode 10; a new upload is needed only for a separately approved future Mobile artifact change. Targeted Play-installed smokes remain appropriate after future changes.

V10 Production publication is complete; Product ID `premium` and active Base Plan ID `monthly` are unchanged and recorded here. No credential, Pub/Sub name, price, currency, or other sensitive production value is defined by this document.
