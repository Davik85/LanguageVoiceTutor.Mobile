...
## Historical controlled validation and current public-production checkpoint (updated 2026-09-03)
...
Historical controlled license-test purchase, renewal, and final expiry are proven. The 2026-09-01 real-money first purchase and backend-owned initial Premium/trial deferral are also proven. Android v8 is now publicly available in Production; remaining evidence is post-release monitoring, not a broader-rollout prerequisite. Actual real-money renewal scheduled for 2026-10-08, pending-payment handling, explicit cancellation before natural expiry, restore on a fresh installation, refund/voided-purchase lifecycle, chargeback lifecycle, and provider-isolation edge cases beyond existing automated/backend coverage remain unobserved.

This is now post-release monitoring: periodically re-review legal/public-policy wording, Google Play Data Safety, test-only controls (including `TestPurchasesEnabled`), pending-refund-review decision, monitoring, and rollback readiness. Signed-out password recovery is in the public `0.1.0+8` / versionCode 8 release; a new upload is needed only for a separately approved Mobile artifact change. Targeted Play-installed smokes remain appropriate after future changes.

Production publication is complete; Product ID `premium` and active Base Plan ID `monthly` are recorded here. No credential, Pub/Sub name, price, currency, or other sensitive production value is defined by this document.
