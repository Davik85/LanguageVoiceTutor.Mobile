...
Current Google Play identity: `com.languagevoicetutor.mobile` is managed in the ORRALEN TECHNOLOGIES LTD Google Play developer account, and Android developer/package verification is confirmed. Google Play Billing, RTDN, and reconciliation remain enabled in production; Android v8 is publicly available.
...
The existing verified release output was exactly `app/build/app/outputs/bundle/release/app-release.aab` (191983753 bytes), with SHA-256 `8C633D4689066BF0BE17B7B7AA266B4049750965092D5642C589DF5F6865A7ED`. It passes `jarsigner` verification, and its embedded upload certificate SHA-1 is `60:A8:13:5D:A6:B1:72:00:F2:6A:80:D2:F9:91:A9:01:CC:EB:F8:9B`; its SHA-256 is `36:40:5D:B4:56:47:B2:3C:68:EE:2D:AB:12:21:70:CA:DE:06:11:38:28:D9:9D:02:AB:62:54:33:E2:F5:0B:F7`, exactly matching the Google Play Console Upload key certificate. No new AAB was built during the release-signing audit.
...
An older upload-key reset request may remain in Google Play Console history, but it is not a current blocker: the working upload key matches the existing AAB certificate. The current public Play release is `0.1.0+8` / versionCode 8. It includes signed-out password recovery and Android Restore Credentials support. VersionCode 5 remains historical billing-lineage evidence only.
...
The earlier Stage 1 checkpoint is superseded by the current baseline: 453 messages per selectable catalog; `flutter gen-l10n` and `flutter analyze` passed; 479 Flutter tests passed; and the debug APK build succeeded. The `flutter_timezone` Kotlin Gradle warning remains non-blocking.
