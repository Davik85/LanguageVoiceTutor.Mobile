...
## 2026-09-20 Android Production v10 public-release checkpoint

Android `0.1.0+10` / versionCode `10` is publicly active in Google Play Production for package `com.languagevoicetutor.mobile`. The release source is `3389786c16d676dae2a66a9cdf73367780babcad` (`Fix Free Conversation context handling`). The signed release artifact is `app/build/app/outputs/bundle/release/app-release.aab` (196468474 bytes, SHA-256 `45B2552ACE707A94772AC0739BF7B6E8632A8C994A54D33C3759DDF79B842C0C`); `jarsigner` reported `jar verified`, and the embedded upload certificate SHA-256 is `36:40:5D:B4:56:47:B2:3C:68:EE:2D:AB:12:21:70:CA:DE:06:11:38:28:D9:9D:02:AB:62:54:33:E2:F5:0B:F7`. Target SDK remains 36.

The final v10 artifact followed a clean-repository release gate: `flutter clean`, dependency and localization generation, analysis, the complete Flutter suite with concurrency 1, a clean-source check, release AAB build, Android version verification, SHA-256 and `jarsigner` checks, upload-certificate verification, and final clean-repository / `HEAD == origin/main` verification all passed. This record intentionally does not invent a total test count.

V10 added Orralen hero artwork to Login/Register over the existing application gradient with a soft transition, removed the redundant auth AppBar title and old separate Login Orralen logo/wordmark block, and retained the localized three-sentence welcome copy. Authentication, registration, password recovery, scrolling, keyboard behavior, and the backend authentication contract were unchanged. This is an authentication-onboarding presentation update, not a claim of a completed product-wide rebrand.

The earlier `fefbac3f366920b980f831ddd55c18e2734471a0` phase-boundary fix remains in force: only initial `setupContextSelection` performs scenario selection, while Free Conversation starts in `activeRoleplay` and uses normal voice and Hint paths. V10 source `3389786c16d676dae2a66a9cdf73367780babcad` made an additional narrow request-boundary correction so ordinary Free Conversation learner text with no committed context remains the normal `UserMessage` and is not also sent as `selectedContextTitle`. Real CMS scenario selection and real custom-context flows remain unchanged; Conversation Mode production rendering did not change. Focused affected suites and physical Android smoke for Free Conversation plus an ordinary guided lesson with a selected situation passed, followed by the full v10 release gate. No backend, API, or CMS change was required.

The known practice-reminder scheduling/reconciliation problem remains open and non-critical after v10; v10 did not implement or reverify a reminder fix. It is deferred to a separately approved future Mobile maintenance release, with verification still required for successful rescheduling, preservation or recovery after replacement-scheduling failure, actual Android alarm/delivery behavior, reboot/timezone behavior where applicable, and actionable failure diagnostics. No exact-alarm or other unverified root cause is claimed.

## Historical 2026-09-06 Android Production v9 review checkpoint
...
Documentation checkpoint: 2026-09-20. Repository main source resolves to `3389786c16d676dae2a66a9cdf73367780babcad` (`Fix Free Conversation context handling`), and `0.1.0+10` / versionCode 10 is publicly active in Google Play Production.

ORRALEN is the operating/company/master brand presented on the public website and legal/company surfaces, and the operating company is ORRALEN TECHNOLOGIES LTD. Language Voice Tutor remains the current product/application name. V10 presents new Orralen hero artwork on the authentication onboarding surface, but this Mobile client has not completed a product-wide rebrand; other visible branding, launcher/in-app assets, and user-facing strings remain governed by their existing product surfaces until a separate audit and approved implementation pass.
...
Current Google Play identity: `com.languagevoicetutor.mobile` is managed in the ORRALEN TECHNOLOGIES LTD Google Play developer account, and Android developer/package verification is confirmed. Google Play Billing, RTDN, and reconciliation remain enabled in production; Android v10 is publicly active.
...
The historical v8 release-signing audit verified `app/build/app/outputs/bundle/release/app-release.aab` (191983753 bytes), with SHA-256 `8C633D4689066BF0BE17B7B7AA266B4049750965092D5642C589DF5F6865A7ED`. It passed `jarsigner` verification, and its embedded upload certificate SHA-1 is `60:A8:13:5D:A6:B1:72:00:F2:6A:80:D2:F9:91:A9:01:CC:EB:F8:9B`; its SHA-256 is `36:40:5D:B4:56:47:B2:3C:68:EE:2D:AB:12:21:70:CA:DE:06:11:38:28:D9:9D:02:AB:62:54:33:E2:F5:0B:F7`, exactly matching the Google Play Console Upload key certificate. The current v10 artifact facts are recorded in the 2026-09-20 checkpoint above.

Google Play Console has `com.languagevoicetutor.mobile` registered, and Android developer verification is confirmed. No additional package registration or key registration is currently required. Historical pre-public-release Internal-testing coverage passed on five Samsung and Huawei Android devices. Installation from Google Play, startup in the system language, correct native-language and interface-language defaults, registration, and restoration of backend-owned settings, Progress, and History for existing accounts all worked.

Physical testing also confirmed lesson start and expected scenario completion, Conversation Mode, speech recognition with automatic message sending, tutor voice playback, Hints, Translation, Feedback, Summary, Progress, achievements, study-language switching without mixed-language lesson openings, general and topic-specific achievements, tutor avatar and voice changes, Settings History and Progress views, password change, and password recovery. Background/resume, force-close/reopen, temporary network loss/recovery, and microphone-permission denial/recovery also passed.

One bounded non-blocking quality item remains: after microphone permission is denied, Lesson Chat keeps a persistent microphone-denied/open-settings message visible. A future artifact change should show that warning only when the learner attempts to use the microphone while permission remains denied. This does not block the public release.

An older upload-key reset request may remain in Google Play Console history, but it is not a current blocker: the working upload key matches the existing AAB certificate. The current public Play release is `0.1.0+10` / versionCode 10. Signed-out password recovery and Android Restore Credentials support were introduced earlier and remain present. VersionCode 5 remains historical billing-lineage evidence only.

## Interface localization and Android first-run defaults

Mobile has fourteen selectable interface locales: `en`, `ru`, `es`, `fr`, `de`, `it`, `pt-PT`, `bg`, `hr`, `sr-Latn`, `pl`, `ja`, `ko`, and `ar`. Each selectable ARB catalog has 454 messages, including the localized authentication welcome message. Flutter `gen-l10n` also generates generic `pt` and `sr` fallback variants; these are internal fallbacks, not additional selectable application locales (16 generated variants in total). Localization groups were added in `e6b3b8c` (Croatian, Serbian, Polish) and `cd799c3` (Japanese, Korean, Arabic); Android first-run defaults were added in `312ca245` (`feat: derive first-run languages from Android`).

`studyLanguage` selects the practiced lesson language, transcription, and tutor-audio behavior; `nativeLanguage` is the translation target; and `explanationLanguage` selects the application interface locale. Physical application geometry deliberately remains left-to-right for every locale, including Arabic: Arabic catalog text is localized, but navigation order, arrows, control order, learner/tutor placement, and screen geometry are not globally mirrored.

At first run, Mobile independently resolves interface and native defaults from Android's ordered preferred locales, falling back to English for each. Splash and Login use the detected interface locale before authentication. After authentication, backend `UserSettings` are authoritative: an existing session or login applies saved `explanationLanguage` without a PUT. New registration fetches created settings once and updates only `nativeLanguage` and `explanationLanguage`, preserving study language, voice, speed, conversation mode, tutor, and level. Failures in that follow-up settings work do not fail authentication or retry registration.

Automated verification completed: `flutter gen-l10n`, `flutter analyze`, the 512-test Flutter suite, and `flutter build apk --debug` passed. The known `flutter_timezone` Kotlin Gradle Plugin future-compatibility warning remains non-blocking. Physical clean-install verification of Android-derived defaults remains pending.

Lesson-selection localization is presentation-only. Canonical IDs, English titles/descriptions, `lessonContentId`, and request values remain unchanged; topic-to-situation navigation uses stable topic IDs, unknown future catalog IDs fall back to canonical text, and translated labels are never backend identifiers. `LessonStartSelection` reconstructs backend-facing values from the authoritative canonical catalog using stable IDs, so caller-provided translated/copied display objects cannot change the request. `StartLessonSessionRequest` and the runtime `scenarioKey` are invariant across all selectable interface locales; the runtime key remains the canonical `lessonContentId`, including Free Conversation.

The accepted start flow remains **Home -> Choose Topic -> Choose Situation -> Lesson**. Home loads backend-owned `UserSettings.currentLevel`, resolves its canonical `LessonOption`, and opens Choose Topic directly. Level selection exists only in **Settings -> Learning**; there is no Choose Level screen, route, or runtime step.
...
Mobile lessons support the six study languages end to end: English, French, German, Portuguese, Spanish, and Italian. Study language is resolved from one centralized definition and remains separate from native/translation language and the fourteen-locale interface implementation. Canonical CMS IDs, English titles, and runtime scenario keys remain unchanged, while backend/CMS own tutor behavior and generated replies.

Localized lesson setup is now backend-owned. The lesson-start response supplies authoritative `localizedSetup` only when it is complete for the selected non-English study language: a nonblank template, exact placeholder set, and exact stable-ID context-title coverage. Mobile parses that projection safely as nullable. When valid it renders only the user display name into the backend template, preserves all other formatting, resolves context titles by stable ID without mixed-language output, and keeps English canonical. When the projection is absent or invalid, Mobile uses the target-language packaged fallback rather than canonical English. Blank-name greetings, numeric/local/canonical/alias context selection, custom context, voice flow, and confirmation title resolution retain their existing behavior. Owner manual Android verification of the localized opening and confirmation flow completed successfully. The implementation is commit `01d6226` (`Use backend localized lesson setup in Mobile`); no Play store build, signing, upload, or release was performed.
...
The earlier Stage 1 checkpoint is superseded by the current baseline: 454 messages per selectable catalog. The final v10 release gate completed localization generation, analysis, the full Flutter suite with concurrency 1, and the signed release-AAB checks successfully; no unverified total test count is asserted here. The `flutter_timezone` Kotlin Gradle warning remains non-blocking.
...
The Google Play billing bridge is configured in the single normal Mobile runtime for controlled Internal testing with Product ID `premium` and Base Plan ID `monthly`; backend processing, RTDN, and reconciliation are configured separately. Controlled purchase, backend-owned Premium, renewal reconciliation, and final expiry are verified. Remaining billing work is pending-payment, explicit-cancellation, fresh-install restore, real-money, refund/voided-purchase, chargeback, provider-isolation, and broader-rollout validation. Notifications V1 is local-only, with product-settings reminders enabled by default at 09:00 and 20:00 in the device local timezone; both times are editable and all reminders can be disabled. It uses neither Firebase nor backend/push state. Android notification permission is still required, must be requested only after the learner sees product value, and denial must not cause a prompt on every launch. Device-local schedules must retain their local-time semantics across timezone changes and be restored after reboot where Android requires it; cross-device lesson completion cannot always suppress a local reminder.

Premium remains backend-owned and backend-verified: a local button, purchase callback, or verified Play result never unlocks Premium by itself. Mobile submits tokens and optionally refreshes backend `SubscriptionStatus`; Google Play, Paddle, trial, and manual-admin feed the same provider-neutral Premium calculation. Controlled Internal-testing purchase, renewal, and final-expiry validation are complete; public rollout and the real-money first-purchase path are also complete. Later lifecycle observations remain post-release monitoring.
...
The original sequence delivered reminders and Premium/billing foundations before localization. Interface localization is now implemented for the fourteen selectable locales. Account, entitlement, access decisions, lesson runtime, transcription, TTS, and AI behavior remain backend-owned.
