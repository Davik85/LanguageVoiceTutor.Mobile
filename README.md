...
Settings has stable visible **Account**, **Learning**, **Audio**, and **Connection status** advanced area, with **Save settings** visible and tested. **Learning** reads and saves the backend-owned account `CurrentLevel`; the dropdown reuses the centralized Mobile `lessonLevels` labels and sends canonical uppercase values. Home **Start lesson** loads that backend setting, resolves the matching `LessonOption`, and opens **Choose Topic** directly. Choose Level has been removed from the normal flow. Settings also reads and saves `selectedTutorId`; tutor voice remains separate. CMS-published level profiles remain the source of language complexity, correction and hint behavior, answer length, and lesson/final-turn timing. No backend deployment is required for this Mobile navigation change, and physical Android validation remains pending.
