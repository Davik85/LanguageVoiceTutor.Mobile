...
The Stage 1 checkpoint verification from `app/` records: `dart format --set-exit-if-changed lib test` checked 129 files with no changes; `flutter gen-l10n` passed; `flutter analyze` reported no issues; the focused Home/selection/runtime suite passed 113 tests; the full Flutter suite passed 382 tests; and `flutter build apk --debug` succeeded. All five ARB catalogs contain 277 messages with identical keys. Stale Choose Level and voice-disclosure scans found no active screen, route, or learner-facing disclosure usage, and Desktop remained clean. The build retains the non-blocking `flutter_timezone` warning about a future Kotlin Gradle built-in migration.
...
The long-term approved interface-language list remains `en`, `es`, `fr`, `de`, `it`, `pt`, `ru`, `pl`, `ar`, `ja`, `ko`, `sr`, `hr`, and `bg`, separate from the six study languages. Stage 1 currently implements Flutter ARB/`gen-l10n` support for `en`, `ru`, `es`, `fr`, and `de`; unsupported saved interface values display English without rewriting the backend setting. Localization never translates AI replies, learner messages, backend-generated content, CMS identifiers, canonical scenario keys, internal IDs, or backend data. The remaining languages, Arabic right-to-left verification, and remaining static screens are later work.
...
The original sequence delivered reminders and Premium/billing foundations before localization. Stage 1 interface localization is now an intentionally partial five-locale checkpoint; completing the remaining static screens and expanding to the full approved language list remain separate work. Account, entitlement, access decisions, lesson runtime, transcription, TTS, and AI behavior remain backend-owned.
