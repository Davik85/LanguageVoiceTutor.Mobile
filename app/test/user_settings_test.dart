...
void main() {
  ...
  test('settings service GET and PUT success with fakes', () async {
    final api = RecordingApiClient(
      getResponse: (_, __) => const ApiResponse(statusCode: 200, body: settingsJson),
      putResponse: (_, __, ___) =>
          const ApiResponse(statusCode: 200, body: settingsJson),
    );
    final service = AuthService(
      apiClient: api,
      storage: MemoryStorage(accessToken: 'token'),
    );
    ...
    expect(api.requests.map((request) => '${request.method} ${request.path} ${request.token}'), [
      'GET /api/me/settings token',
      'PUT /api/me/settings token',
    ]);
  });
  ...
  test('settings service failure is sanitized', () async {
    final api = RecordingApiClient(
      ...
      putResponse: (_, __, ___) => const ApiResponse(statusCode: 500, body: '{}'),
    );
