...
class FakeTutorOptionsService extends TutorOptionsService {
  ...
  @override
  Future<TutorOptions> fetchTutorOptions() async => const TutorOptions(
        tutors: [
...
Future<void> _pumpLoadedSettings(WidgetTester tester, FakeAuthService auth) async {
...
void main() {
  testWidgets('settings screen loaded state shows stable product sections',
      (tester) async {
    await _pumpLoadedSettings(tester, FakeAuthService());
  ...
  testWidgets('settings screen has save action and no user level selection',
      (tester) async {
    await _pumpLoadedSettings(tester, FakeAuthService());
  ...
  testWidgets('settings screen save success shows friendly message',
    ...
    await _pumpLoadedSettings(tester, auth);
  ...
  testWidgets('settings screen friendly failure hides raw backend details',
    ...
    expect(find.textContaining('raw stack trace'), findsNothing);
  });
}
